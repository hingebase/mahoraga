...
async def _fetch_repo_data(  # noqa: PLR0913
    *,
    ...
    repo_data_list = await rattler.rattler.py_fetch_repo_data(  # pyright: ignore[reportUnknownMemberType, reportUnknownVariableType]
        [channel._channel for channel in channels],  # pyright: ignore[reportPrivateUsage, reportUnknownMemberType]
        [platform._inner for platform in platforms],  # pyright: ignore[reportPrivateUsage, reportUnknownMemberType]
        cache_path,
        callback,
        client and client._client,  # pyright: ignore[reportPrivateUsage, reportUnknownMemberType]
        fetch_options._into_py(),  # pyright: ignore[reportPrivateUsage, reportUnknownMemberType]
    )
    return [
        rattler.SparseRepoData._from_py_sparse_repo_data(  # pyright: ignore[reportPrivateUsage, reportUnknownMemberType]
